...
CREATE TABLE IF NOT EXISTS `sisdf`.`sala` (
  ...
  `num_sala` VARCHAR(6) NOT NULL,
  `nome` VARCHAR(150) NOT NULL,
  `ramal` VARCHAR(6) NULL,
  `bloco` VARCHAR(3) NULL,
  `andar` VARCHAR(15) NULL,
  `id_responsavel_fk` INT NOT NULL,
  `in_lista_ramais` TINYINT(1) NULL,
  `ativo` TINYINT(1) NULL COMMENT 'Flag que alimenta ou não a lista de ramais\ntrue = 1\nfalse = 0 / NULL',
  PRIMARY KEY (`id_sala`),
  UNIQUE INDEX `num_sala_UNIQUE` (`num_sala` ASC),
  INDEX `fk_sala_usuario1_idx` (`id_responsavel_fk` ASC),
  CONSTRAINT `fk_sala_usuario1`
    FOREIGN KEY (`id_responsavel_fk`)
