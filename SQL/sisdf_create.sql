...
CREATE TABLE IF NOT EXISTS `sisdf`.`usuario` (
  ...
  PRIMARY KEY (`id_usuario`, `id_cargo_fk`),
...
CREATE TABLE IF NOT EXISTS `sisdf`.`ordem_servico` (
  ...
  `atendente` VARCHAR(150) NULL COMMENT 'Inserido a partir da tabela de usuários',
  `resp_secao` VARCHAR(150) NOT NULL,
  `resp_local_os` VARCHAR(150) NOT NULL COMMENT 'Responsável pela sala, no momento do atendimento',
...
CREATE TABLE IF NOT EXISTS `sisdf`.`responsavel` (
  ...
  `id_sala_resp_fk` INT NOT NULL,
  PRIMARY KEY (`id_resp_fk`, `id_sala_resp_fk`),
  INDEX `fk_responsavel_usuario1_idx` (`id_resp_fk` ASC),
  INDEX `fk_responsavel_sala1_idx` (`id_sala_resp_fk` ASC),
